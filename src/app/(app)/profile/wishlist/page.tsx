            ...
            {wishlistItems.map((item) => {
              ...
              const cardContent = (
                <div className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.03] border border-white/5 hover:bg-white/[0.06] transition-colors">
                  {/* 와인 이미지 */}
                  <div className="w-10 h-14 rounded-lg overflow-hidden bg-zinc-900 flex-shrink-0">
                    <img src={d.image} alt={item.name_ko} className="w-full h-full object-contain mix-blend-lighten" />
                  </div>

                  {/* 와인 정보 */}
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-zinc-200 font-medium truncate">{item.name_ko}</p>
                    {item.name_en && (
                    ...
                    {d.grapes && (
                      <p className="text-[11px] text-zinc-600 mt-0.5 truncate">{d.grapes}</p>
                    )}
                  </div>
