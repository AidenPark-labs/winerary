...
const TYPE_COLOR: Record<string, string> = {
  red: "bg-rose-950 text-rose-300",
  white: "bg-amber-950 text-amber-300",
  rose: "bg-pink-950 text-pink-300",
  sparkling: "bg-sky-950 text-sky-300",
  fortified: "bg-purple-950 text-purple-300",
    ...
    <div className="flex flex-col flex-1">
      ...
      {!records || records.length === 0 ? (
        <div className="flex flex-col flex-1 items-center justify-center gap-4 text-center px-8">
          ...
          <Link
            href="/diary/new"
            className="px-6 py-3 rounded-xl bg-rose-700 hover:bg-rose-600 text-white font-semibold transition-colors"
          >
            첫 와인 기록하기
          </Link>
        </div>
            ...
            <li key={record.id}>
              ...
              >
                {record.label_image_url ? (
                  ...
                  <div className="w-16 h-20 rounded-lg bg-zinc-800 flex items-center justify-center text-2xl flex-shrink-0">
                    🍷
                  </div>
                )}
                <div className="flex flex-col gap-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    {record.type && (
                      <span className={`text-xs px-2 py-0.5 rounded-full ${TYPE_COLOR[record.type] ?? TYPE_COLOR.other}`}>
                        {TYPE_LABEL[record.type]}
                      </span>
                    )}
                    {record.vintage && (
                      <span className="text-xs text-zinc-500">{record.vintage}</span>
                    )}
                  </div>
                  <p className="font-semibold truncate">{record.name}</p>
                  {record.producer && (
                    <p className="text-sm text-zinc-400 truncate">{record.producer}</p>
                  )}
