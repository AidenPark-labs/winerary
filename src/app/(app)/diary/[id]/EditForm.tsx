    ...
    <>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 p-4 rounded-2xl bg-zinc-900 border border-zinc-800">
        ...
        <div className="flex flex-col gap-2">
          <label className="text-sm text-zinc-400">페어링 음식</label>
          <div className="flex gap-2">
            <input value={foodInput} onChange={(e) => setFoodInput(e.target.value)}
              ...
              placeholder="음식 추가" className={iCls} />
            <button type="button" onClick={() => { if (foodInput.trim()) { setFoods((f) => [...f, foodInput.trim()]); setFoodInput(""); } }}
              className="px-4 py-3 rounded-xl bg-zinc-700 hover:bg-zinc-600 text-zinc-200 text-sm">추가</button>
          </div>
          {foods.length > 0 && (
